...
_RPI_STEP_THEMES = {
    "Data": {"color": "#5E2C91", "bg": "#E7D6F6", "soft": "#F3EAFB"},
    "Architecture": {"color": "#5E2C91", "bg": "#E7D6F6", "soft": "#F3EAFB"},
    "Training": {"color": "#5E2C91", "bg": "#E7D6F6", "soft": "#F3EAFB"},
}
...
def _render_step_nav(active_step: str, next_disabled: bool = False) -> None:
    ...
    with nav_cols[0]:
        if st.button(
            ...
            key=f"rpi_prev_{active_step.lower()}",
        ):
    ...
    with nav_cols[1]:
        if st.button(
            ...
            use_container_width=True,
            key=f"rpi_next_{active_step.lower()}",
        ):
            _set_builder_step(_WORKFLOW_STEPS[current_idx + 1])


def _render_workflow_band(step: str) -> None:
    theme = _RPI_STEP_THEMES[step]
...
RNAFM_OPTIONS = {
    "RNA-FM (640-dim, default)": ("multimolecule/rnafm", 640),
}
...
def _generate_demo_csv() -> str:
    """Generate synthetic RPI demo data (50 pairs) with a fixed random seed."""
    rng = random.Random(42)

    RNA_BASES   = list("AUGC")
    RNA_WEIGHTS = [0.25, 0.22, 0.28, 0.25]   # slight G-bias
    ...
    # Positive pairs: RBP-like proteins with AU-rich RNA motifs
    AU_RICH = ["AUUUA", "AUUUUA", "UUAUUU", "AAUAAA", "UAAUUA"]

    def _rand_rna(length: int, enriched: bool) -> str:
        seq = "".join(rng.choices(RNA_BASES, weights=RNA_WEIGHTS, k=length))
        if enriched and rng.random() < 0.7:
            # embed one AU-rich element at a random position
            motif = rng.choice(AU_RICH)
    ...
    for label in ([1] * 25 + [0] * 25):
        for _ in range(300):
            rna  = _rand_rna(rng.randint(40, 120), enriched=(label == 1))
            prot = _rand_prot(rng.randint(60, 150))
            key  = (rna[:20], prot[:20])
            if key not in used:
                used.add(key)
                rows.append({"rna_sequence": rna, "protein_sequence": prot, "label": label})
                break

    rng.shuffle(rows)
    lines = ["rna_sequence,protein_sequence,label"]
    for r in rows:
        lines.append(f"{r['rna_sequence']},{r['protein_sequence']},{r['label']}")
    return "\n".join(lines) + "\n"
...
def _arch_figure(layer_configs: list, rna_dim: int, prot_dim: int) -> plt.Figure:
    BG        = "#F5F0FF"   # soft violet tint
    input_dim = rna_dim + prot_dim

    entries = [{
        "name": "Input",
        "dim":  input_dim,
        "sub":  f"concat(RNA,prot)\n{input_dim}-dim",
    }]
    cur_dim = input_dim
...
def _validate(df, col_rna, col_prot, col_label):
    errors, warnings = [], []

    for col in [col_rna, col_prot, col_label]:
        n = df[col].isnull().sum()
        if n:
    ...
    # RNA: only A/U/G/C/N (also accept T — will be converted to U)
    sample_rna = df[col_rna].head(200)
    bad_rna = sample_rna.astype(str).str.strip().str.upper() \
                .str.replace("T", "U", regex=False) \
                .apply(lambda s: not bool(_VALID_RNA.match(s)) or len(s) == 0)
    if bad_rna.sum():
        first = sample_rna[bad_rna].iloc[0][:30]
        errors.append(
            f"Column **{col_rna}** contains {int(bad_rna.sum())} sequence(s) with invalid "
            f"characters (first: `{first}...`). Only A, U, G, C (and T, converted to U) accepted."
        )
...
st.caption("Train a classifier to predict binding between RNA sequences and RNA-binding proteins.")
st.divider()
...
builder_step_theme = _RPI_STEP_THEMES[active_step]
st.markdown(
    f"""
    <style>
    :root {{
...
col_rna = data_context.get("col_rna", "rna_sequence")
col_prot = data_context.get("col_prot", "protein_sequence")
col_label = data_context.get("col_label", "label")
mean_rna_len = float(data_context.get("mean_rna_len", 0.0))
mean_prot_len = float(data_context.get("mean_prot_len", 0.0))
stats = data_context.get("stats", {"rows": 0, "unique_rna": 0, "unique_prots": 0, "n_pos": 0, "n_neg": 0})
...
default_rna_label = list(RNAFM_OPTIONS.keys())[0]
rna_label = st.session_state.get("rpi_rna_label", default_rna_label)
if rna_label not in RNAFM_OPTIONS:
    rna_label = default_rna_label
rna_model_name, rna_dim = RNAFM_OPTIONS[rna_label]
default_esm2_label = list(ESM2_OPTIONS.keys())[1]
esm2_label = st.session_state.get("rpi_esm2_label", default_esm2_label)
...
chunk_dtype = st.session_state.get("rpi_chunk_dtype", "float16")
sequence_mode = embedding_representation == "chunked"
input_dim = max(rna_dim, esm_dim) if sequence_mode else rna_dim + esm_dim
st.session_state.setdefault("rpi_layers", default_layers())
st.session_state.setdefault("_rpi_lid", 2)
layers: list = st.session_state["rpi_layers"]
...
if active_step == "Data":
    ...
    st.session_state.setdefault("rpi_demo_loaded",   False)
    st.session_state.setdefault("rpi_uploader_key",  0)
    _EDITED_DF_KEY = "rpi_edited_df"
    _EDITED_FLAG_KEY = "rpi_data_edited"
    _SOURCE_KEY = "rpi_data_source"
    _POS_COUNT_KEY = "rpi_positive_count"
    _NEG_COUNT_KEY = "rpi_negative_count"
    _SAMPLING_SIGNATURE_KEY = "rpi_sampling_signature"
    ...
    with col_dl:
        st.download_button(
            ...
            file_name="demo_rpi_train.csv", mime="text/csv",
            help="Download the synthetic demo CSV for offline inspection.",
            use_container_width=True,
        )
    with col_ex:
        if st.button("Load Example Data",
                     help="Loads 50 synthetic RNA-protein pairs to test the workflow.",
                     use_container_width=True):
            clear_edited_df(_EDITED_DF_KEY, _EDITED_FLAG_KEY)
            st.session_state["rpi_demo_loaded"] = True
            st.session_state["rpi_uploader_key"] += 1
            st.rerun()
    with col_reset:
        if st.button("Reset", help="Clear loaded data and reset the model builder.", use_container_width=True):
            clear_edited_df(_EDITED_DF_KEY, _EDITED_FLAG_KEY)
            st.session_state["rpi_demo_loaded"] = False
            st.session_state["rpi_uploader_key"] += 1
            reset_model_builder_state(
                "rpi_layers",
                "_rpi_lid",
                widget_prefix="rpi_",
                new_layer_type_key="rpi_new_layer_type",
                model_defaults={
                    "rpi_rna_label": list(RNAFM_OPTIONS.keys())[0],
                    "rpi_esm2_label": list(ESM2_OPTIONS.keys())[1],
                },
            )
            st.rerun()
    ...
    st.caption(
        ...
        "You will map them to **RNA Sequence**, **Protein Sequence**, and **Label (0/1)** below. "
        "DNA sequences (T) are automatically converted to RNA (U)."
    )

    uploaded = st.file_uploader(
        "Upload CSV", type=["csv"], label_visibility="collapsed",
        key=f"rpi_uploader_{st.session_state['rpi_uploader_key']}",
    )

    demo_loaded = st.session_state.get("rpi_demo_loaded", False)
    data_ready  = False
    raw_df      = None
    ...
    if uploaded is not None:
        ...
        if st.session_state.get(_SOURCE_KEY) != source_id:
            clear_edited_df(_EDITED_DF_KEY, _EDITED_FLAG_KEY)
            st.session_state[_SOURCE_KEY] = source_id
        st.info("Using example data (50 synthetic RNA-protein pairs). Upload your own CSV to override.")
    ...
    if data_ready:
        ...
        render_edited_download(raw_df, _EDITED_FLAG_KEY, "edited_rpi_train.csv")

    if not data_ready:
        st.markdown("""
        <style>
        div[data-testid="stVerticalBlock"] > div:has(.rpi-grey-marker) ~ div {
            opacity: 0.4;
            pointer-events: none;
            user-select: none;
        }
        </style>
        <span class="rpi-grey-marker"></span>
        """, unsafe_allow_html=True)
    ...
    _placeholder_cols = ["rna_sequence", "protein_sequence", "label"]
    _sel_cols = raw_df.columns.tolist() if data_ready else _placeholder_cols

    mc1, mc2, mc3 = st.columns(3)
    with mc1:
        col_rna   = st.selectbox("RNA Sequence",     _sel_cols, index=0)
    with mc2:
    ...
    mean_rna_len  = 0.0
    mean_prot_len = 0.0
    stats         = {"rows": 0, "unique_rna": 0, "unique_prots": 0, "n_pos": 0, "n_neg": 0}
    n_pairs_use   = 0
    ...
    if data_ready:
        if col_rna == col_prot:
            st.error("RNA Sequence and Protein Sequence cannot be the same column.")
            st.stop()

        errors, warnings, stats = _validate(raw_df, col_rna, col_prot, col_label)
        affected_mask = build_recoverable_row_mask(
            raw_df,
            col_rna,
            col_prot,
            col_label,
            lambda value: bool(_VALID_RNA.match(value.upper().replace("T", "U"))),
        ...
        leakage_msgs = leakage_warnings("rpi", mapped_df)
        if leakage_msgs:
        ...
        st.success(
            ...
            f"{stats['unique_rna']:,} unique RNA sequences · "
            f"{stats['unique_prots']:,} unique proteins · "
            f"{stats['n_pos']:,} positive · {stats['n_neg']:,} negative · "
            f"avg RNA length {int(mean_rna_len):,} nt · "
            f"avg protein length {int(mean_prot_len):,} aa"
        )

        preview = raw_df[[col_rna, col_prot, col_label]].head(5).copy()
        preview.columns = ["rna_sequence (preview)", "protein_sequence (preview)", "label"]
        preview["rna_sequence (preview)"]     = preview["rna_sequence (preview)"].astype(str).str[:50] + "…"
        ...
        if sampling_over_cap:
            st.error(
                f"Selected {sample_counts.selected_total:,} pairs, but the RPI limit is "
    ...
    st.session_state[_DATA_CONTEXT_KEY] = {
        ...
        "col_prot": col_prot,
        "col_label": col_label,
        "mean_rna_len": mean_rna_len,
        "mean_prot_len": mean_prot_len,
        "stats": stats,
...
if active_step == "Architecture":
    ...
    with emb1:
        ...
        rna_label = st.selectbox(
            "RNA-FM model",
            list(RNAFM_OPTIONS.keys()),
            index=0,
            key="rpi_rna_label",
            help="Encodes RNA sequences into fixed-length embeddings using RNA-FM.",
        )
        rna_model_name, rna_dim = RNAFM_OPTIONS[rna_label]
        st.caption(f"`{rna_model_name}` · **{rna_dim}**-dim output")
    ...
    embedding_representation = st.radio(
        ...
        horizontal=True,
        key="rpi_embedding_representation",
    ...
    for i, layer in enumerate(layers):
        ...
        with st.container(border=True):
            ...
            with hdr_cols[1]:
                if i > 0:
                    if st.button("↑", key=f"rpi_up_{lid}", help="Move up"):
                        to_move = (i, "up")
            with hdr_cols[2]:
                if i < len(layers) - 1:
                    if st.button("↓", key=f"rpi_dn_{lid}", help="Move down"):
                        to_move = (i, "down")
            with hdr_cols[3]:
                if st.button("✕", key=f"rpi_rm_{lid}", help="Remove layer"):
                    to_remove = i
            ...
            if lt == "linear":
                ...
                with c1:
                    layer["hidden_dim"] = st.number_input(
                        ...
                        value=int(layer.get("hidden_dim", 256)), key=f"rpi_hd_{lid}",
                    )
                with c2:
                ...
                with c1:
                    layer["out_channels"] = st.number_input(
                        ...
                        value=int(layer.get("out_channels", 64)), key=f"rpi_och_{lid}",
                    )
                with c2:
                    ...
                    layer["kernel_size"] = st.selectbox(
                        ...
                        index=KERNEL_OPTIONS.index(ks_val), key=f"rpi_ks_{lid}",
                    )
                with c3:
                    layer["activation"] = st.selectbox(
                        "activation", ACT_OPTIONS,
                ...
                with c1:
                    layer["hidden_size"] = st.number_input(
                        ...
                        value=int(layer.get("hidden_size", 128)), key=f"rpi_hs_{lid}",
                    )
                with c2:
                    layer["num_layers"] = st.number_input(
                        "num_layers", min_value=1, max_value=3,
                        value=int(layer.get("num_layers", 1)), key=f"rpi_nl_{lid}",
                    )
                with c3:
                    layer["dropout"] = st.slider(
                        "dropout", 0.0, 0.7, float(layer.get("dropout", 0.3)), 0.05,
                ...
                with c1:
                    ...
                    layer["d_model"] = st.number_input(
                        ...
                        value=d_model_val, key=f"rpi_dm_{lid}",
                    )
                with c2:
                    ...
                    layer["nhead"] = st.selectbox(
                        "nhead", valid_nh,
                        index=valid_nh.index(nhead_val), key=f"rpi_nh_{lid}",
                    )
                with c3:
                    layer["num_layers"] = st.number_input(
                        "num_layers", min_value=1, max_value=4,
                        value=int(layer.get("num_layers", 2)), key=f"rpi_nl_{lid}",
                    )
                with c4:
                    d_now   = int(layer["d_model"])
                    ...
                    layer["dim_feedforward"] = st.selectbox(
                        ...
                        index=ff_opts.index(ff_val), key=f"rpi_ff_{lid}",
                    )
                with c5:
                    layer["dropout"] = st.slider(
    ...
    if to_remove is not None:
        ...
        st.session_state["rpi_layers"] = layers
        st.rerun()
    ...
    with add_cols[0]:
        new_type = st.selectbox("Layer type", LAYER_TYPES, key="rpi_new_layer_type",
                                label_visibility="collapsed")
    with add_cols[1]:
        if st.button("Add Layer", use_container_width=True):
            new_id = st.session_state["_rpi_lid"]
            st.session_state["_rpi_lid"] += 1
            defaults_by_type = {
...
if active_step == "Training":
    ...
    est = _estimate_time(
        n_pairs_use, stats["unique_rna"], mean_rna_len,
    ...
    if st.button("Submit Training Job", type="primary", use_container_width=True, disabled=submit_disabled):
        ...
        send_df = raw_df[[col_rna, col_prot, col_label]].copy()
        send_df.columns = ["rna_sequence", "protein_sequence", "label"]
        send_df["label"] = send_df["label"].astype(float).astype(int)
        # Convert T→U in RNA column before sending
        send_df["rna_sequence"] = (
            send_df["rna_sequence"].astype(str).str.strip().str.upper()
            .str.replace("T", "U", regex=False)
        )
