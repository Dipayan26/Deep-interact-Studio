...
MAX_PDI_DNA_BP = 512
MAX_PDI_RESIDUES = 512

_WORKFLOW_STEPS = ["Data", "Architecture", "Training"]
_PDI_STEP_THEMES = {
    "Data": {"color": "#B55F04", "bg": "#FFE0BE", "soft": "#FFF0DE"},
    "Architecture": {"color": "#A95600", "bg": "#FFE0BE", "soft": "#FFF0DE"},
    "Training": {"color": "#A95600", "bg": "#FFE0BE", "soft": "#FFF0DE"},
}
...
def _render_step_nav(active_step: str, next_disabled: bool = False) -> None:
    ...
    with nav_cols[0]:
        if st.button(
            ...
            key=f"pdi_prev_{active_step.lower()}",
        ):
    ...
    with nav_cols[1]:
        if st.button(
            ...
            use_container_width=True,
            key=f"pdi_next_{active_step.lower()}",
        ):
            _set_builder_step(_WORKFLOW_STEPS[current_idx + 1])


def _render_workflow_band(step: str) -> None:
    theme = _PDI_STEP_THEMES[step]
...
def _estimate_time(n_pairs: int, n_unique_dna: int, mean_dna_len: float,
    ...
    dna_embed_s  = (n_unique_dna   / 16) * 0.20
...
builder_step_theme = _PDI_STEP_THEMES[active_step]
st.markdown(
    f"""
    <style>
    :root {{
...
col_dna = data_context.get("col_dna", "dna_sequence")
col_prot = data_context.get("col_prot", "protein_sequence")
col_label = data_context.get("col_label", "label")
mean_dna_len = float(data_context.get("mean_dna_len", 0.0))
mean_prot_len = float(data_context.get("mean_prot_len", 0.0))
stats = data_context.get("stats", {"rows": 0, "unique_dna": 0, "unique_prots": 0, "n_pos": 0, "n_neg": 0})
...
default_dna_label = list(DNABERT_OPTIONS.keys())[0]
dna_label = st.session_state.get("pdi_dna_label", default_dna_label)
if dna_label not in DNABERT_OPTIONS:
    dna_label = default_dna_label
dna_model_name, dna_dim = DNABERT_OPTIONS[dna_label]
default_esm2_label = list(ESM2_OPTIONS.keys())[1]
esm2_label = st.session_state.get("pdi_esm2_label", default_esm2_label)
if esm2_label not in ESM2_OPTIONS:
    esm2_label = default_esm2_label
esm_model_name, esm_dim = ESM2_OPTIONS[esm2_label]
embedding_representation = st.session_state.get("pdi_embedding_representation", "pooled")
dna_chunk_max_len = MAX_PDI_DNA_BP
protein_chunk_max_len = MAX_PDI_RESIDUES
dna_num_chunks = int(st.session_state.get("pdi_dna_chunks", 8))
protein_num_chunks = int(st.session_state.get("pdi_protein_chunks", 8))
chunk_dtype = st.session_state.get("pdi_chunk_dtype", "float16")
sequence_mode = embedding_representation == "chunked"
input_dim = max(dna_dim, esm_dim) if sequence_mode else dna_dim + esm_dim
...
n_param = _total_param_count(
    ...
    layer_configs_display,
    sequence_mode=sequence_mode,
    projection_dims=(dna_dim, esm_dim) if sequence_mode else None,
) if layers else 0
...
if active_step == "Data":
    ...
    _POS_COUNT_KEY = "pdi_positive_count"
    _NEG_COUNT_KEY = "pdi_negative_count"
    _SAMPLING_SIGNATURE_KEY = "pdi_sampling_signature"
    ...
    if data_ready:
        ...
        mapped_df = mapped_training_frame(
            raw_df,
            [col_dna, col_prot, col_label],
            ["dna_sequence", "protein_sequence", "label"],
        )
        leakage_msgs = leakage_warnings("pdi", mapped_df)
        if leakage_msgs:
        ...
        long_mask = (
            long_sequence_row_mask(raw_df, [col_dna], MAX_PDI_DNA_BP)
            | long_sequence_row_mask(raw_df, [col_prot], MAX_PDI_RESIDUES)
        )
        long_row_count = int(long_mask.sum())
        if long_row_count:
            dna_lens_for_warning = raw_df[col_dna].astype(str).str.strip().str.len()
            prot_lens_for_warning = raw_df[col_prot].astype(str).str.strip().str.len()
            max_dna_len_detected = int(dna_lens_for_warning.max())
            max_prot_len_detected = int(prot_lens_for_warning.max())
            st.warning(
                f"{long_row_count:,} pair row(s) contain DNA longer than {MAX_PDI_DNA_BP} bp "
                f"(max {max_dna_len_detected:,}) or protein longer than {MAX_PDI_RESIDUES} residues "
                f"(max {max_prot_len_detected:,}). Resolve this before training."
            )
            long_preview = raw_df.loc[long_mask, [col_dna, col_prot, col_label]].head(5).copy()
            long_preview["DNA length"] = dna_lens_for_warning.loc[long_mask].head(5).to_numpy()
            long_preview["Protein length"] = prot_lens_for_warning.loc[long_mask].head(5).to_numpy()
            long_preview[col_dna] = long_preview[col_dna].astype(str).str[:50] + "…"
            ...
            with fix_cols[0]:
                if st.button(
                    f"Trim long DNA/proteins to {MAX_PDI_DNA_BP}",
                    key="pdi_trim_long_sequences",
                    use_container_width=True,
                ):
                    trimmed = trim_sequence_columns(raw_df, [col_dna, col_prot], MAX_PDI_DNA_BP)
                    apply_edited_df(trimmed, _EDITED_DF_KEY, _EDITED_FLAG_KEY)
            with fix_cols[1]:
                if st.button(
                    f"Remove {long_row_count:,} long-sequence row(s)",
                    key="pdi_remove_long_sequence_rows",
                    use_container_width=True,
                ):
        ...
        invalid_embedding_mask = invalid_embedding_row_mask(
            raw_df,
            {
                col_dna: lambda value: bool(_VALID_DNA.match(value.upper())),
        ...
        if render_invalid_embedding_cleanup(
            ...
            _EDITED_FLAG_KEY,
            "pdi",
            "DNA/protein sequences outside the allowed alphabets or whitespace that can hinder embedding",
        ):
            st.stop()
        ...
        if sampling_over_cap:
            st.error(
                f"Selected {sample_counts.selected_total:,} pairs, but the PDI limit is "
...
if active_step == "Architecture":
    ...
    embedding_representation = st.radio(
        ...
        horizontal=True,
        key="pdi_embedding_representation",
        help="Pooled stores one vector per side. Chunked stores local window embeddings for both DNA and protein.",
    )
    ...
    if embedding_representation == "chunked":
        ...
        with cc1:
            dna_num_chunks = st.selectbox("DNA chunks", [4, 8, 16, 32], index=1, key="pdi_dna_chunks")
        with cc2:
            protein_num_chunks = st.selectbox("Protein chunks", [4, 8, 16, 32], index=1, key="pdi_protein_chunks")
        with cc3:
            chunk_dtype = st.selectbox("Storage precision", ["float16", "float32"], index=0, key="pdi_chunk_dtype")
        st.caption(
            f"Chunked input: {dna_num_chunks} DNA chunks + {protein_num_chunks} protein chunks, "
    ...
    st.caption(
        ...
        + (
            f"shared dims per chunk ({dna_dim} DNABERT and {esm_dim} ESM2 projected)"
            if sequence_mode
            else f"dims ({dna_dim} DNABERT-2 + {esm_dim} ESM2, concatenated)"
        )
    )
    ...
    with arch_info_cols[1]:
        st.caption(
            ...
            + (
                f"{dna_num_chunks} DNABERT chunks + {protein_num_chunks} ESM2 chunks"
                if sequence_mode
                else f"({dna_dim} DNABERT-2 + {esm_dim} ESM2)"
            )
        )
...
if active_step == "Training":
    ...
    est = _estimate_time(
        n_pairs_use, stats["unique_dna"], mean_dna_len,
    ...
    if st.button("Submit Training Job", type="primary", use_container_width=True, disabled=submit_disabled):
        ...
        hp = {
            ...
            "dna_chunk_max_len":    dna_chunk_max_len,
            "dna_num_chunks":       dna_num_chunks,
        ...
        with st.spinner("Submitting..."):
            try:
                ...
                st.session_state["pdi_submitted_run_id"] = data["run_id"]
    ...
    submitted_run_id = st.session_state.get("pdi_submitted_run_id", "")
    if submitted_run_id:
        if st.button("Check Model Results", type="primary", use_container_width=False, key="pdi_check_model_results"):
